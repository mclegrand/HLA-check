...
string alnfile = "alnall";

string nuc_[8] = {"a.aln","b.aln","c.aln","drb.aln","dpa.aln","dpb.aln","dqa.aln","dqb.aln"};
string rslist_ = "rs_all";

string reference_hla_[8] = {"01:01:01:01","07:02:01","01:02:01","01:01:01", "01:01:01","01:01:01","01:01:01","05:01:01:01" };
...
void main_loop(float* scoremin, string& hla1min, string& hla2min, int i,const vector<string> &hlax1, const vector<string> & hlax2, const vector<string> &defx1, const vector<string> &defx2, bool do_all = true){
  ...
  for(int ii=0;ii<hlax1.size();ii++){
    ...
    for(;jj<hlax2.size();jj++){
      ...
      for(int k=0; k<data.size();k++){
        ...
        char h1 = seq1[pos];
        if(h1 == '-'){h1=reftruc[pos];}
        char h2 = seq2[pos];
        if(h2 == '-'){h2=reftruc[pos];}
        if(h2 == '*' && h1=='*'){continue;}
        if(allele3 == '-'){
if((h1!= allele1 && h1 != allele2 && h1 != '*')||(h2!= allele1 && h2 != allele2 && h2 != '*') ){score+=1;continue;}
          if(h1 == '*'){
...
if((h1!= allele1 && h1 != allele2 && h1 != allele3 && h1 != '*')||(h2!= allele1 && h2 != allele2 && h2 != allele3 && h2 != '*') ){score+=1;continue;}
          if(h1 == '*'){
      ...
      scoretot += log((1-score)*0.9+.1);//1-score = "pas mauvais"
      }
      
      if(scoretot > *scoremin){
        (*scoremin)=scoretot;hla1min=hla1;hla2min=hla2;
      }
    }
  }
}//main_loop
...
if(argc<4){printf("prog A data hla\n"); return 1;}

int chosen_hla = -1;
if(!strncmp(argv[1],"A",1))chosen_hla=A;
if(!strncmp(argv[1],"B",1))chosen_hla=B;
if(!strncmp(argv[1],"C",1))chosen_hla=C;
if(!strncmp(argv[1],"DRB1",1))chosen_hla=DRB1;
if(!strncmp(argv[1],"DPA1",1))chosen_hla=DPA1;
if(!strncmp(argv[1],"DPB1",1))chosen_hla=DPB1;
if(!strncmp(argv[1],"DQA1",1))chosen_hla=DQA1;
if(!strncmp(argv[1],"DQB1",1))chosen_hla=DQB1;
if(chosen_hla = -1) printf("HLA must be A,B,C,DRB1,DPA1,DPB1,DQA1, or DQB1\n"); return 1;

ifstream aln_file;
aln_file.open(prefix + alnfile);
if (!aln_file.is_open()){printf("Could not open alignment file\n"); return 1;}
string hla, nm_name, coding_sequence, nm_sequence;
while(hla != argv[1])
    aln_file >> hla >> nm_name >> coding_sequence >> nm_sequence;

ifstream nucfile;
nucfile.open(prefix + nuc_[chosen_hla]);
if (!nucfile.is_open()){printf("Could not open hla sequences definition\n"); return 1;}

ifstream rslist;
rslist.open(prefix + rslist_);
if (!rslist.is_open()){printf("Could not open the rs list\n");return 1;}

ifstream datafile;
datafile.open(argv[2]);
if (!datafile.is_open()){printf("Could not open data file\n"); return 1;}

ifstream hlafile;
hlafile.open(argv[3]);
if (!hlafile.is_open()){printf("Could not open hla file\n"); return 1;}
...
while ( 1 ){
  ...
  for(int i=0;;i++){
    if(nm_sequence[i]=='A' || nm_sequence[i]=='T' || nm_sequence[i]=='G' || nm_sequence[i]=='C'){
      rspos_nm--;
      if(rspos_nm<0){rspos_f=i-1;break;}
    }
  }
  int rspos_nuc =0;
  ...
  for(int i=0;;i++){
    if(reftruc[i]=='A' || reftruc[i]=='T' || reftruc[i]=='G' || reftruc[i]=='C' ){
      rspos_nuc--;
      if(rspos_nuc<0){refpos_nuc2=i;break;}
    }
  }
...
while(!end){
  ...
  if(!getline(datafile,line)){end=true;rs = "rs0";}else{
  stringstream x(line);
  x>>a;x>>rs;x>>a;x>>a;x>>a;x>>a;x>>allele1;x>>allele2;
  }
  if(rs != cur_rs){
    stringstream x,y;
    char a1,a2,a3,a4;
    ...
    switch(oldlines.size()){
      case 1://biallelic
        x.str(oldlines[0]);
        x>>a;x>>cur_rs;x>>a;x>>a;x>>a;x>>a;x>>a1;x>>a2;
      ...
      case 2://triallelic
        ...
        y.str(oldlines[1]);
        x>>a;x>>cur_rs;x>>a;x>>a;x>>a;x>>a;x>>a1;x>>a2;
        y>>a;y>>cur_rs;y>>a;y>>a;y>>a;y>>a;y>>a3;y>>a4;
        data_allele1.push_back(a1);
        data_allele2.push_back(a2);
...
if(data.empty()){printf("data format fail\n");return 1;}
